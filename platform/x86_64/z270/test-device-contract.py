#!/usr/bin/env python3
"""Trusted all-PCI Zone0 contract; not a hardware or isolation test."""
import re
...
bars = [(0xde000000,0x1000000),(0xc0000000,0x10000000),(0xd0000000,0x2000000),
        (0xdf080000,0x4000),(0xdf100000,0x4000),(0xdf200000,0x8000),
...
assert len(rows) == len(inventory) == 19
bdfs = {value[:3] for value in inventory.values()}
assert len(bdfs) == 19
assert bdfs == {(0,d,f) for d,f in [(0,0),(1,0),(0x14,0),(0x16,0),(0x17,0),
 (0x1b,0),(0x1c,0),(0x1c,4),(0x1c,7),(0x1d,0),
 (0x1f,0),(0x1f,2),(0x1f,3),(0x1f,4),(0x1f,6)]} | {(1,0,0),(1,0,1),(4,0,0),(5,0,0)}
...
assert inventory["wifi"][-1] == "rp8" and inventory["asmedia"][-1] == "rp5"
